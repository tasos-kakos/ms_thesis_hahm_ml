...
def process_events(input_path, output_path, max_events=10, onlyOneSector=True):
    ...
    for i, rvec in enumerate(data['MdtChamHitId_stationEta']):
        ...
        station_eta_signed_int = np.array([int.from_bytes(x.encode('latin-1'), 'big',signed=True) for x in station_eta], dtype=np.int32)
        ...
        station_index = np.array([ord(x) if isinstance(x, str) else x for x in station_index])
        ...
        event_variables = ['MdtChamHitId_multiLayer', 'MdtChamHitId_stationEta', 'MdtChamHitId_stationPhi', 'MdtChamHitId_stationIndex', 'MdtChamHitId_tubeLayer', 'MdtChamHitId_tube', 'MdtChamHit_Adc', 'MdtChamHit_Tdc', 'MdtChamHit_ChamberLink', 'MdtChamHit_driftR', 'MdtChamHit_uncertDriftR']
